...
fun createTaskDefinitionFromRequest(request: RegisterTaskDefinitionRequest): TaskDefinition = TaskDefinition()
    .withCompatibilities(request.requiresCompatibilities?.ifEmpty { null })
    .withContainerDefinitions(request.containerDefinitions?.ifEmpty { null })
    .withCpu(request.cpu)
    .withExecutionRoleArn(request.executionRoleArn)
    .withTaskRoleArn(request.taskRoleArn)
    .withFamily(request.family)
    .withInferenceAccelerators(request.inferenceAccelerators?.ifEmpty { null })
    .withIpcMode(request.ipcMode)
    .withMemory(request.memory)
    .withNetworkMode(request.networkMode)
    .withPidMode(request.pidMode)
    .withPlacementConstraints(request.placementConstraints?.ifEmpty { null })
    .withProxyConfiguration(request.proxyConfiguration)
    .withRequiresAttributes()
    .withRequiresCompatibilities(request.requiresCompatibilities?.ifEmpty { null })
    .withRevision(1)
    ...
    .withVolumes(request.volumes?.ifEmpty { null })
